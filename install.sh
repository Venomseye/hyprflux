...
apply_active_theme() {
    ...
    # Generate waybar colors.css using GTK @define-color format
    # CSS custom properties (--var) are NOT supported in GTK3 (used by Waybar)
    ...
    if [[ -f "$dark_theme" ]]; then
        {
            ...
            grep "^#@wb" "$dark_theme" | while IFS= read -r wbline; do
                ...
                [[ -n "$cname" && -n "$cvalue" ]] &&                     printf "@define-color %s %s;\n" "$cname" "$cvalue"
            done
        } > "$colors_dest"
        success "Waybar colors.css generated (GTK @define-color)"
    fi
